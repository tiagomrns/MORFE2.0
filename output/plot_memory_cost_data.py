...
# ============================================
mono_file = "beam_o_11_eps_11_Fmode_1_2026-05-19T11_43_07/benchmark_per_monomial.csv"
...
# ============================================
order_file = "beam_o_11_eps_11_Fmode_1_2026-05-19T11_43_07/benchmark_per_order.csv"
df_order = pd.read_csv(order_file)

# Total memory allocation per order (sum of all three contributions)
df_order['total_alloc_bytes_order'] = (
    df_order['fillrhsG_alloc_bytes'] +
    df_order['fillrhsH_alloc_bytes'] +
    df_order['fillWf_alloc_bytes']
)
cumulative_order_totals = df_order['total_alloc_bytes_order'].cumsum().values
...
ax1.fill_between(global_index + 4, total_cumulative/(1024**3), 0, color='bisque', alpha=1.0)
ax1.plot(global_index + 4, total_cumulative/(1024**3), color='darkorange', linewidth=2, label='Total cumulative memory (GB)')

ax1.fill_between(global_index + 4, cumulative_mono/(1024**3), 0,
                 color='red', alpha=1.0, label='Total cumulative memory (GB)')
...
ax1.plot(x_smooth + 4, poly_orange(x_smooth)/(1024**3),
...
ax2 = ax1.twinx()
ax2.bar(global_index + 4, df_mono['monomial_total_alloc_bytes']/(1024**3),
        width=0.8, color='orange', alpha=0.6, label='Memory per monomial')
ax2.set_ylabel('Memory per monomial (GB)', color='orange')
...
plt.ylim(0, total_cumulative.max()/(1024**3) * 1.05)
plt.grid(False)
plt.show()
# %%
